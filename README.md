...
Agent Pipeline
Symptom Agent → Extracts symptoms, severity, duration, and urgency level
Doctor Agent → Matches symptoms to specialties and recommends top doctors
Scheduling Agent → Generates available appointment slots based on urgency
Summary Agent → Creates comprehensive health summary with recommendations
🚀 Quick Start
Prerequisites
...
Hugging Face Spaces
See README_HUGGINGFACE.md for detailed Hugging Face deployment instructions.

Quick steps:

Create a new Docker Space on Hugging Face
Copy all files to your space
Add secrets: GEMINI_API_KEY, PINECONE_API_KEY
Push and deploy
📊 API Documentation
Health Assessment
POST /api/v1/assess
